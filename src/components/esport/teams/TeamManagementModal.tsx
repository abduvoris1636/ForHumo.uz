...
interface TeamManagementModalProps {
    ...
    onClose: () => void;
    team: Team;
    currentUserId: string; // Added prop for RBAC
...
export function TeamManagementModal({ isOpen, onClose, team, currentUserId, onUpdateTeam, onDeleteTeam }: TeamManagementModalProps) {
        ...
        try {
            const res = await fetch('/api/teams/requests/action', {
                ...
                body: JSON.stringify({
                    ...
                    currentUserId: currentUserId, // The one approving
                    action
                })
            });
    ...
    const handleSendInvite = (playerId: string) => {
        if (!isOwner) return; // Only owner sends invites? Or Captain? Let's assume Owner for now.
                                    ...
                                    <div className="space-y-4 max-w-xs">
                                        ...
                                        <p className="text-zinc-500 text-sm">
                                            {isOwner ?
                                                ...
                                                "Ask your captain to generate an invite key for new members."}
                                        </p>
                                                ...
                                                <div key={req.playerId} className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-3 flex items-center gap-4 hover:border-zinc-700 transition-colors">
                                                    ...
                                                    <div className="flex gap-2">
                                                        {isOwner ? (
                                                            ...
                                                            <span className="text-[10px] text-zinc-600 bg-zinc-800/50 px-2 py-1 rounded">View Only</span>
                                                        )}
                                                    </div>
                                                </div>
                                            );
